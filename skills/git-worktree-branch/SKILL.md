...
description: 新しい作業を開始するときの Git Worktree ブランチ運用。ユーザーが「新しい機能を追加したい」「バグを修正したい」「作業を開始したい」「ブランチを作成したい」と言ったときに使用します。
...
このスキルは、Git Worktree を使って新しい作業ブランチを作成する標準手順を提供します。
...
- 機能追加の場合: `feature/機能名`
  - 例: `feature/add-preview`, `feature/improve-parser`
- バグ修正の場合: `fix/修正内容`
...
```bash
# 機能追加の場合
git worktree add ../myproject-feature feature/add-preview

# バグ修正の場合
git worktree add ../myproject-fix fix/parse-error
```

**Worktree の仕組み説明:**
- メインリポジトリと同じ階層に新しいディレクトリが作成される
- 新しいブランチが自動的に作成され、そのブランチにチェックアウトされる
- Worktree ディレクトリ名: `<プロジェクト名>-{ブランチ種別}`
