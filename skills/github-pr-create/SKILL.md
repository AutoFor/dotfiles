...
**適用条件:** 現在のブランチ名が `^[0-9]{8}-[0-9]{6}$` にマッチする場合のみ実行する。
（例: `20260210-143052` のようなタイムスタンプのみのブランチ名）

それ以外のブランチ名（`feature/xxx`、`fix/xxx` 等）の場合はこのステップをスキップする。

**新ブランチ名:** `issue-<Issue番号>-<元のタイムスタンプ>`
（例: `issue-123-20260210-143052`）
...
**変更後の表示:**
```
🔄 ブランチ名を変更しました: 20260210-143052 → issue-123-20260210-143052
```
...
**Q: タイムスタンプブランチとは何ですか？**
A: `/git-worktree-branch` で即時作成される `YYYYMMDD-HHMMSS` 形式のブランチ名です（例: `20260210-143052`）。PR作成時にIssue番号が確定するため、そのタイミングで `issue-XX-YYYYMMDD-HHMMSS` にリネームされます。
