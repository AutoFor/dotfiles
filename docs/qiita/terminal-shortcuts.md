...
| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `<leader> →Space` (1 回目) | 録音開始。右ステータスにマイクアイコンが出る（**灰色=マイク起動待ち、赤=録音中**。赤くなってから話すと頭が切れない）。以降、話すたびにフレーズ単位で文字が流れ込む | Space = 話す間 |
...
- **キーを押した直後の発話が切れる**のは録音プロセスの起動時間（pwsh 起動 + C# コンパイル）のせい。コンパイル結果は `%TEMP%` に DLL キャッシュされるので、**初回だけ 2〜3 秒、以降は 1 秒弱**で録音が始まる。確実に拾いたいときはアイコンが赤くなってから話し始める
