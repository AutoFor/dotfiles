...
| `Alt+k` / `Alt+j` | ペインの**上端/下端まで来てからもう 1 回押すと**前 / 次のセッションへ抜ける（移動先では反対側の端のペインに着地）。ペイン移動と地続きなので、縦に押し続けると全セッションを一周できる。途中のペインを飛ばして 1 発で移りたいときは `Ctrl+q → (` / `)` | k=上=前, j=下=次 の vim 方向 |
...
tmux はセッションを名前の昇順でしか並べない（`Alt+k/j` でセッションを跨ぐときの順もこれ）ため、
...
| `Ctrl+Shift+w` | **閉じたタブを開き直す**。タブ名・ペインのレイアウト・各ペインの cwd とペイン名が戻り、claude / codex / kimi が動いていたペインは同じ会話で再開する。連打すると閉じた順に遡れる（直近 20 件まで） | `Ctrl+w` の取り消し（Shift＝逆操作） |
...
| `<leader> →X` | **閉じたペインを開き直す**（`Ctrl+q → Shift+X`）。cwd・ペイン名・claude / codex / kimi の会話・分割レイアウトごと元の位置に戻る。連打で閉じた順に遡れる。ウィンドウ版が `<leader> →W` なのと対 | e**X**hume（掘り起こす） |
...
| `Alt+z`（`<leader> →z`） | 現在のペインをズーム（トグル）。`Alt+z` はプレフィックス不要の 1 打鍵版 | **z**oom |
...
| ペイン間移動（vim 風） | `Alt+h/j/k/l`（端まで来たら横はタブ、縦はセッションへ抜ける） | 同上 |
