...
| `<leader> →T` | **現在のセッション名を変更**（先頭の並び順番号は維持されるので、入力するのは番号無しの名前だけ。`Ctrl+q → $`） | **T**itle（`<leader> →t` のタブ名変更のセッション版＝大文字） |
...
番号を振って並びを操作できるようにしている。番号は自動で採番・詰め直しされるので手で付ける必要はなく、
`Alt+Shift+,` / `Alt+Shift+.` で隣と番号を入れ替えるのが「並び替え」にあたる。
