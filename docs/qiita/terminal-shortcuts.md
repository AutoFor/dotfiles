...
| `Alt+←/→`（`Ctrl+←/→`） | 左/右のペインへ移動。**端で止まらずタブ（tmux ウィンドウ）を跨ぐ**（一番左で `Alt+←` → 前のタブの一番右のペイン、一番右で `Alt+→` → 次のタブの一番左のペイン）。タブの端でも巻き戻るので押し続けると全タブの全ペインを一周できる。右列が上下分割されたタブに左から入ると下側のペインに着く（tmux が指定できる角が `{bottom-right}` のため）。`Alt+↑/↓` はタブを跨がず、そのタブ内の上下移動だけ | 矢印キー = 方向 |
