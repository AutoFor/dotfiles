...
| `Alt+p` | 現在のペインに名前を付ける（空 Enter で解除して自動表示に戻る。`<leader> →p` / `Ctrl+b → Shift+T` でも可） | **p**ane name |
...
スクラッチパッドと違い、ファイルとして残したい一時メモ用。メモ用フォルダで nvim を立ち上げて使う想定（ファイルはカレントディレクトリに作られる）。
