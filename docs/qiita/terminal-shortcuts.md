...
- **小さいマイクでも拾う**: 無音判定のしきい値は固定値ではなく、録音開始直後 600ms の環境音から実測したノイズフロアの 2 倍（120〜300 でクランプ）を自動採用する。上限は旧来の固定値 300 なので、自動決定によって従来より厳しくなる（声を拾わなくなる）ことはない。マイクの入力ボリュームが小さい環境でも「声あり」判定が立つ。計測中は下限値で判定するので頭は切れない。決定したしきい値は `%TEMP%\wezterm-voice.log` に残る
- **音量の正規化**: Groq に送る前にチャンクのピークを揃えて増幅する（目標ピーク 22000 = フルスケールの約 67%、上限 8 倍）。小音量のまま投げると Whisper 側の認識精度も落ちるため
- 自動しきい値が合わないとき（環境ノイズで区切られない / 声を拾わない）は `-SilenceThreshold` に値を渡すと自動決定を止めてその固定値を使う
