...
| `<leader> →m` | **現在のペインを別のウィンドウ/セッションへ移動**（一覧から移動先を選ぶ。選んだウィンドウに合流する。`Ctrl+q → m`） | **m**ove（ペイン単位＝小文字） |
| `<leader> →M` | **現在のウィンドウ（＝このタブ）を丸ごと別のセッションへ移動して追従**（一覧から移動先セッションを選ぶ。タブ名・ペイン名はそのまま維持）。`<leader> →m` のウィンドウ単位版。`Ctrl+q → Shift+M` | **M**ove（ウィンドウ単位＝大文字） |
...
> 同じ理由で、ウィンドウごとの移動（`<leader> →M`）・ペインの切り出し（`Ctrl+q → !`）・タブの入れ替え（`Alt+,` / `Alt+.`）でも自動保存が走る。
...
| このペインを別のウィンドウ/セッションへ移動 | `m`（小文字）→ 一覧から選択 |
