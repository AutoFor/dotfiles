...
また、**画面下部の tmux ステータスバーの左下に、アクティブペインの cwd のフォルダ名（青）と git ブランチ名（緑）が常時表示される**（リポジトリ外はフォルダ名のみ）。ウィンドウ一覧・右下（セッション名+日時）は WezTerm のタブバーと重複するため、モバイル（グループセッション）のときだけ下部バーに表示される。
