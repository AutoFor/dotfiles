...
また、**画面下部の tmux ステータスバーの左下に、接続表示（通常は緑の `devbox`、アクティブペインで ssh 実行中は赤の `ssh`）、アクティブペインの cwd のフルパス（青、ホームは `~` 省略）、git ブランチ名（緑）が常時表示される**（リポジトリ外はパスのみ。長いパスは末尾優先で先頭を `…` 省略）。WezTerm 右上は普段は空で、LEADER・音声入力中などの一時的なインジケータだけが出る（音声入力の録音状態は Windows 側にしか無いため右上に残している）。ウィンドウ一覧・右下（セッション名+日時）は WezTerm のタブバーと重複するため、モバイル（グループセッション）のときだけ下部バーに表示される。
