...
モバイルでの操作は tmux の prefix 直打ち（Termius のキーボードバーの `ctrl` を使う）:
...
プレフィックスなしで使えるタブ切替も用意している（Termius は C-Tab を送信できないための代替）:

| 操作 | キー |
|------|------|
| 前/次のタブ | `Shift+←` / `Shift+→` |
| タブ番号で移動 | `Alt+1`〜`Alt+9`（Termius は設定で Option を Esc 扱いにすると使える） |
