...
| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `<leader> →Space` (1 回目) | 録音開始。**画面中央に波形付き「録音中」のフローティング表示**が出る（実測のマイクレベルで動くので、拾えているかが一目で分かる）。右ステータスにも 🎤 が出る。以降、話すたびにフレーズ単位で文字が流れ込む | Space = 話す間 |
...
- 無音判定がおかしいとき（環境ノイズで区切られない / 声を拾わない）は `-SilenceThreshold`（既定 300）を調整。中央の波形表示が声に反応していなければマイク・しきい値を疑う
- 中央のフローティング表示は Windows の最前面ウィンドウ（`voice-overlay.ps1`）で、フォーカスを奪わない（tmux の `display-popup` はキー入力を奪って send-text の文字起こしを食ってしまうため使っていない）。不要なら `voice-input.ps1` の `-NoOverlay`
- **キーを押した直後の発話が切れる**のは録音プロセスの起動時間（pwsh 起動 + C# コンパイル）のせい。コンパイル結果は `%TEMP%` に DLL キャッシュされるので、**初回だけ 2〜3 秒、以降は 1 秒弱**で録音が始まる。確実に拾いたいときは中央の波形表示が出てから話し始める
