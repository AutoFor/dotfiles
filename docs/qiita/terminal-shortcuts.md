...
Windows の WezTerm からクラウド開発サーバー（Azure devbox）の tmux + Zsh に接続する構成で使っている、自分がよく忘れるショートカット・コマンド・操作手順をまとめたメモです。
...
（旧構成の WSL2 は #212 で引退。WSL 前提の記述は削除済み）

---

## WezTerm + tmux

Leader キーは `Ctrl+q`（2秒タイムアウト）。tmux の prefix も `Ctrl+q` に揃えてある（`Ctrl+b` も従来どおり効く）。
...
WezTerm はただの表示器 + SSH クライアントで、以下のタブ/ペイン系キーはすべて tmux の
prefix シーケンスに自動変換される（ローカルの PowerShell ペインでは何もしない）。
階層の対応: WezTerm の Window/Tab/Pane ≒ tmux の **Session / Window / Pane**。

### Session（旧 Workspace 相当）

セッション = タブ構成ごと切り替えられる作業文脈の単位。リモート側に永続し、切断・再起動・別端末からの接続でもそのまま復帰する。

| ショートカット / コマンド | 動作 | 由来 |
|------------------------|------|------|
| `tm` | main セッションに attach（無ければ作成）。シェルで実行 | **t**mux **m**ain |
| `tm <名前>` | 別セッションに attach（無ければ作成） | 同上 |
| `<leader> →s` | セッション/ウィンドウ一覧から選択して切り替え | **s**ession |
| `<leader> →n` | 新しいセッションを作る（名前を聞かれ、そのまま移動する。`Ctrl+q → Shift+N`） | **n**ew |
| `<leader> →m` | **現在のペインを別のウィンドウ/セッションへ移動**（一覧から移動先を選ぶ。選んだウィンドウに合流する。`Ctrl+q → Shift+M`） | **m**ove |
| `<leader> →g` | **現在のウィンドウ（＝このタブ）を丸ごと別のセッションへ移動して追従**（一覧から移動先セッションを選ぶ。タブ名・ペイン名はそのまま維持）。`<leader> →m` のウィンドウ単位版。`Ctrl+q → g` | **g**o |
| `Ctrl+q → (` / `)` | 前 / 次のセッションへ | tmux 標準 |
| `Ctrl+q → d` | セッションから detach（セッションは生存継続） | **d**etach |

> `<leader> →m` でペインを動かすと、ペインの位置（セッション名 / ウィンドウ番号 / ペイン番号）が変わる。
> ペイン名と Claude Code の会話 ID はこの位置をキーに保存しているので、移動したまま放置すると
> 次回の復元で名前や会話が元の位置に付いてしまう。**移動直後に自動で保存し直すようにしてあるので手動保存は不要**。
> 同じ理由で、ウィンドウごとの移動（`<leader> →g`）・ペインの切り出し（`Ctrl+q → !`）・タブの入れ替え（`Alt+,` / `Alt+.`）でも自動保存が走る。

> tmux 標準の「直前のセッションへ戻る」`prefix + L` は使えない。`L` はペインサイズ調整に割り当ててあるため。

### Tab（実体は tmux ウィンドウ）

タブ一覧は WezTerm のタブバー（ヘッダー）に従来どおりの見た目（矢羽根 + 小さめ UI フォント）で表示される。仕組みは tmux がウィンドウ構成の変化を `wezterm-tabs-sync` で WezTerm に通知し、WezTerm がタブ風に描画する**表示専用**の連携。クリックでは切り替えられないので `Ctrl+Tab` / `Ctrl+数字` を使う。tmux 自身のステータスラインは非表示。
...
複数ペインのウィンドウでは、各ペインの上辺ボーダーに `番号: 名前` の**名前バー**が常時表示される
（アクティブペインは青太字）。名前を付けていないペインは実行中のコマンド名（`claude` / `nvim` / `zsh` 等）が
自動で出るので、1 画面のどのペインで何が動いているか一望できる。1 ペインだけのウィンドウでは非表示。

| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `<leader> →r` | ペインを左右に分割 | **r**ight（左右に分かれる） |
| `<leader> →d` | ペインを上下に分割 | **d**own（上下に分かれる） |
| `<leader> →x` | 現在のペインを閉じる（確認なしで即クローズ） | × 印（消す） |
| `Alt+h/l/k/j` | 左/右/上/下のペインへ移動（端まで来たら nvim 等のアプリへ透過） | vim の hjkl |
...
| `<leader> →q` | ペイン番号をオーバーレイ表示して選択 | tmux の `prefix+q` と同じ |
| `Ctrl+b → H/J/K/L` | ペインサイズを調整（連打可。マウスの境界ドラッグでも可） | vim hjkl の大文字 |
| `Ctrl+b → Shift+E` | ペインサイズを均等に整える（分割構成はそのまま、幅/高さだけ揃える。3分割の幅を揃えたいときはこれ） | **E**ven（均等） |
| `Ctrl+b → Alt+1`〜`Alt+5` | プリセットレイアウトに一発切替（1=左右に均等 / 2=上下に均等 / 3=メイン上+残り横並び / 4=メイン左+残り縦並び / 5=タイル状） | tmux 標準レイアウト |
| `<leader> →j` | 最後に通知が来た Claude Code のペインへジャンプ | **j**ump |
| `<leader> →f` | ファイラー（yazi）をどのペインにも属さない浮遊ポップアップで開く（アクティブペインの cwd 起点。Enter でファイルをポップアップ内 nvim で開き、`q` で閉じる） | **f**iler |
...
### iPad / iPhone からの接続（Tailscale + Termius）

前提: **Tailscale アプリ**（VPN トグル ON、devbox と同じアカウント）と **Termius**（無料版で十分）。
SSH 鍵は端末ごとに Termius の Keychain で生成（ED25519）し、公開鍵を devbox の `~/.ssh/authorized_keys` に登録する（端末ごとに別鍵にすると紛失時にその端末だけ無効化できる）。

| 項目 | 値 |
|---|---|
| Address | `100.126.96.27`（devbox の Tailscale IP。不変） |
| Username | `azureuser`（タイプミス注意） |
| Key | その端末で生成した鍵 |

接続したら `tm` で main セッションに入る。`tm` は**グループセッション**（ウィンドウ共有・ビュー独立）で attach するため:

- 画面下部に tmux のステータスライン（タブ一覧）が表示される（PC は WezTerm タブバーがあるので非表示のまま）
- PC と**別のタブを独立して表示**できる（タブの中身・増減は全端末で共有）
- 切断するとビュー用セッションは自動削除される（タブは devbox に残る）

モバイルでの操作は「プレフィックスなしキー」「タッチ」「prefix 直打ち」の3通り。

#### プレフィックスなしキー（ハードウェアキーボード向け）

PC の `Ctrl+Tab` / `Ctrl+数字` は WezTerm がキーを prefix シーケンスに変換して送る仕組みのため、
Termius からはそもそも送信できない。代わりに標準シーケンスで届くキーを tmux 側に用意している:

| 操作 | キー | 備考 |
|------|------|------|
| 前/次のタブ | `Shift+←` / `Shift+→` | どのアプリでも届く。代償としてペイン内アプリに Shift+矢印は届かない |
| ペイン間移動 | `Ctrl+矢印` | どのアプリでも届く。代償としてペイン内アプリの Ctrl+矢印（単語ジャンプ等）は使えない |
| タブ番号で移動 | `Alt+1`〜`Alt+9` | Termius は設定で Option を Esc（Meta）扱いにすると使える |
| ペイン間移動（vim 風） | `Alt+h/l/k/j`（`Alt+矢印` でも可） | 同上 |

#### タッチ操作（`mouse on` で tmux が反応する）

| 操作 | 動作 |
|------|------|
| ペインをタップ | そのペインへフォーカス移動 |
| ステータスラインのタブ名をタップ | そのタブへ切り替え |
| ペイン境界をドラッグ | ペインサイズ調整 |
| 2本指スクロール | copy-mode に入って履歴をさかのぼる |

#### prefix 直打ち（`Ctrl+q` または `Ctrl+b` のあとに。Termius のキーボードバーの `ctrl` を使う）

ソフトウェアキーボードだけの iPhone でも打てる、どの端末でも確実に効く経路:
...
| ペイン間移動 | 矢印キー |
| ペイン番号を表示して選択 | `q` → 番号 |
| ペインサイズ調整 | `H/J/K/L`（連打可） |
| ペインサイズを均等に整える | `E`（Shift+e） |
| ズーム（1ペイン全画面をトグル） | `z` |
| ペインに名前を付ける | `T`（Shift+t。空 Enter で解除） |
| ペインを閉じる | `x`（確認なし） |
| セッション/ウィンドウ一覧から選択 | `s` |
| 新しいセッションを作る（名前を聞かれる） | `N`（Shift+n） |
| このペインを別のウィンドウ/セッションへ移動 | `M`（Shift+m）→ 一覧から選択 |
| このウィンドウを丸ごと別のセッションへ移動して追従 | `g` → 一覧からセッションを選択（タブ名・ペイン名は維持） |
...
`<leader> →[` は tmux の copy-mode にブリッジされる（ローカルペインでは WezTerm copy mode）。
マウスホイールでのスクロールでも自動で copy-mode に入る。キーは vi 準拠:
...
| `v` | 文字単位の選択開始 | **v**isual |
| `V` | 行単位の選択開始 | **V**isual line |
| `y` | 選択範囲をコピーして終了（OSC 52 で Windows のクリップボードに載る） | **y**ank |
| `q / Escape` | コピーモードを終了 | **q**uit / 脱出 |

#### マウスでのコピー（tmux 構成での正解）

| 操作 | 動作 |
|------|------|
| ドラッグして離す | **離した瞬間に Windows クリップボードへ入る**（OSC 52）。その後の `Ctrl+Shift+C` は不要 — 押すと WezTerm 側の空選択で上書きされるので押さないこと |
| ダブルクリック | カーソル下の**スペース区切りトークン**を即コピー。URL やパスは `-` `@` `?` を含んでも、**複数行に折り返していても**一発で全体が取れる |
| トリプルクリック | その**行**を即コピー（折り返し行は繋いで 1 行になる） |
...
| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `<leader> →Space` (1 回目) | 録音開始。右ステータスにマイクアイコンが出て、色が実際の音声に連動する（**灰色=マイク起動待ち、青=録音中だが無音、赤=声を拾っている**。青になってから話すと頭が切れない。話して赤くならないならマイク・しきい値を疑う）。以降、話すたびにフレーズ単位で文字が流れ込む | Space = 話す間 |
...
- **辞書**は `windows/voice-dictionary.txt`（書式はファイル内コメント参照）。「単語のみ」の行は Whisper への用語ヒント（認識がその表記に寄る）、「誤=正」の行は文字起こし結果の確実な置換。Groq クラウド側に辞書機能は無いのでこの二段構えで代替している。リポジトリ管理なので devbox から編集して push → pull でも反映できる（スクリプトが毎回読み直す）
- **辞書への自動提案**: 音声入力を終了するたび、そのセッションの文字起こしを Groq の LLM（`llama-3.3-70b-versatile`）が見て、誤変換らしき用語を「`# 誤=正`」の**コメント状態**で辞書末尾に追記・自動コミットする（トーストで件数を通知）。採用するなら行頭の `# ` を外す、不要なら行ごと削除。コメントのままなら置換には使われないので誤提案が入力を壊すことはない。無効化は `voice-input.ps1` の `-SuggestModel ''`
- 文字起こし全文は devbox の `~/.cache/voice-transcripts.log` にもミラーされるので、devbox の Claude Code に「誤変換の傾向を見て辞書を整理して」と頼む運用もできる
...
- **キーを押した直後の発話が切れる**のは録音プロセスの起動時間（pwsh 起動 + C# コンパイル）のせい。コンパイル結果は `%TEMP%` に DLL キャッシュされるので、**初回だけ 2〜3 秒、以降は 1 秒弱**で録音が始まる。確実に拾いたいときはアイコンが青（録音中）になってから話し始める
- おまけ: `voice-input.ps1` に `-Overlay` を付けると、画面中央に実測マイクレベルで動く波形付きフローティング表示（`voice-overlay.ps1`、フォーカスは奪わない）が出る。既定はオフ（アイコンのみ運用）
- 一時的な文字起こし失敗（レート制限・ネットワーク断）はそのフレーズだけ捨てて**録音を継続**する（トーストで知らせる。連続 3 回失敗で終了）。録音プロセスが落ちた場合も約 10 秒でアイコンが消えて状態が自動リセットされ、次の `<leader> →Space` が普通に効く
...
#### WezTerm の外へのプッシュ通知（ntfy）

devbox の `~/.config/ntfy-topic`（git 管理外）にトピック名を置くと、`notify.sh` は
**手元の WezTerm でトーストが出せないときだけ** [ntfy.sh](https://ntfy.sh) にプッシュを送る。
WezTerm で見ている最中は上記の OSC トーストだけになり、二重通知しない。

「WezTerm でトーストが出せない」の判定は 2 通り:

| 通知元 | 判定 |
|--------|------|
| tmux 内 | 自セッション（グループセッションなら同一グループ）に **クライアントが attach していない** = 席を外している |
| tmux 外 | OSC の**書き込み先 tty が無い** = Claude Code アプリからの SSH セッション等、そもそも WezTerm を経由していない |
...
- **Windows**: `windows/bin/ntfy-listen.ps1` が購読して BurntToast でトーストにする。
  `windows/bin/register-ntfy-listen.ps1` でログオン時常駐に登録（管理者不要）。
  トピック名は `%USERPROFILE%\.config\ntfy-topic` に devbox と同じ値を置く
- **iPhone / Apple Watch**: ntfy アプリで同じトピックを購読すれば Apple Watch にも自動でミラーされる
...
タブバー右端のステータスに **接続状態・アクティブなキーテーブル・日時** を常時表示している。

接続状態はラベルと色で見分けられる（#214 で tmux 移行後）:

| 表示 | 色 | 意味 |
|------|----|------|
| `devbox` | 緑 | **ssh + tmux の通常運用**（セッションはリモート tmux が保持。切断しても生き残る） |
| `MUX:devbox` | オレンジ | 旧 wezterm mux ドメイン（切り分け用フォールバック） |
| `SSH:ホスト名` | 赤 | devbox からさらに別ホストへ素の `ssh` をした状態（切断するとセッションも消える） |
| `local` | 緑 | ローカル（Windows） |

仕組み: 通常運用は SSH ドメイン名（`devbox-tmux`）で判定。加えてシェルが OSC 7 で報告するホスト名（`pane:get_current_working_dir().host`）を併用し、devbox から先へのネスト SSH を検出している（リモート側のシェルが OSC 7 を発行していることが前提。dotfiles の zsh 設定なら tmux 越しでも発行される）。
...
| コマンド | 実行する場所 | 動作 | 由来 |
|---------|------------|------|------|
| `tssh user@host` | 接続元 | trzsz でラップした SSH 接続（`trzsz -d ssh` のエイリアス） | **t**rzsz + **ssh** |
| `trz` | リモート | 接続元にファイル選択ダイアログが開き、選んだファイルをアップロード | **t**rzsz **r**eceive → **z**（rz 互換） |
| `trz -d` | リモート | ディレクトリごとアップロード | **d**irectory |
| `tsz <file>` | リモート | ファイルを接続元にダウンロード | **t**rzsz **s**end → **z**（sz 互換） |

> **注意**: trzsz は「`ssh` コマンドを打つ側」にインストールする（GUI ダイアログには zenity 等が必要）。
> 現構成（Windows → devbox）では、Windows ⇔ devbox のファイル転送は `scp devbox:...` で行うのが簡単
> （`ssh devbox` エイリアスが scp にも効く）。trzsz は devbox からさらに先のホストに入るときの選択肢。
...
`ghq create <name>` の既定 owner は `ai-foo`。
別の組織に作るときは `owner/repo-name` で指定する。

| コマンド | 動作 |
|---------|------|
| `ghq create repo-name` | `ai-foo/repo-name` に作成 |
| `ghq create owner/repo-name` | 指定した owner に作成 |
...
実体は `linux/.local/bin/pplx`（devbox 上で実行）。
...
`branch` / `checkout` / `switch` は役割が似て見えるが、担当が違う。
今は「作る・一覧を見る」は `branch`、「切り替える」は `switch` を基本にすると整理しやすい。
`checkout` は昔の万能コマンドで、ブランチ切替とファイル復元の両方を担っていたため混乱しやすい。

| コマンド | 動作 | ひとことでいうと |
|---------|------|------------------|
| `git branch` | ブランチを一覧・作成・削除する | ブランチ管理 |
| `git checkout` | ブランチ移動 / 作成 / ファイル復元までできる | 昔の万能コマンド |
| `git switch` | ブランチ切替専用 | 切替担当 |

#### 一覧・作成・削除 - `branch`

| コマンド | 動作 | 由来 |
|---------|------|------|
| `git branch` | ブランチ一覧を表示 | branch = 枝 |
| `git branch feature/test` | 新しいブランチを作成する。今いるブランチはそのまま | 作るだけで移動しない |
| `git branch -d feature/test` | ブランチを削除する | `-d` = delete |

#### 旧コマンド - `checkout`

| コマンド | 動作 | 由来 |
|---------|------|------|
| `git checkout -b feature/test` | ブランチを作ってそのまま切り替える | `-b` = branch |

#### 切替 - `switch`

| コマンド | 動作 | 由来 |
|---------|------|------|
| `git switch main` | ブランチを切り替える | switch = 切り替え |
| `git switch -c feature/test` | ブランチを作って切り替える | `-c` = create |

> 今のおすすめは、**作成・一覧は `git branch`、移動は `git switch`**。
> `git checkout` は古い記事や README にまだよく出るので、「読むための知識」として押さえておくと混乱しにくい。

### リモート

`origin` = リモートリポジトリ（GitHub 等）につけた短い別名。`git remote -v` で確認できる。

#### 初回接続

| コマンド | 動作 | ひとことでいうと |
|---------|------|------------------|
| `gh repo create AutoFor/TicketRadar --private --source=. --remote=origin` | GitHub の repo を作って、ローカルの `origin` も設定する | GitHub につなげる初回コマンド |

`gh repo create ... --source=. --remote=origin` で GitHub 側の repo 作成とローカルの接続をまとめて行う。

#### 接続確認

| コマンド | 動作 | ひとことでいうと |
|---------|------|------------------|
| `git remote -v` | `origin` などのリモート URL を表示する | `-v` = verbose（詳しく表示） |
| `git fetch origin` | GitHub のリモートへ実際にアクセスできるか確認する | 通信確認 |

`git remote -v` で GitHub の URL が出ていれば、リモート設定はできている。`git fetch origin` が通れば、実際に GitHub と接続できていると見てよい。

#### 毎回の push

| コマンド | 動作 | ひとことでいうと |
|---------|------|------------------|
| `git push` | ローカルの変更をリモートへ送る | 普段の送信 |
| `git push -u origin main` | 初回 push と upstream 設定をまとめて実行する | 最初の送信と追跡設定 |

`git push` は毎回の送信に使う。初回だけ `git push -u origin main` にすると、upstream も同時に設定できる。

#### 同期 / 強制上書き

`pull` = 足す（ローカルを残しながら統合）、`reset --hard` = 捨てて合わせる（ローカル作業が消える）。

| コマンド | 動作 | ローカル変更 |
|---------|------|------------|
| `git pull` | fetch + merge（または rebase）でリモートを統合 | 残る（コンフリクトあり得る） |
| `git reset --hard origin/main` | 現在ブランチを `origin/main` と同じ状態に強制上書き | **消える**（未コミット・add 済み・ローカルコミットすべて） |

よくある使い分け：

| 状況 | コマンド |
|------|---------|
| 普段の更新 | `git pull --rebase` |
| ローカルがぐちゃぐちゃで GitHub 状態に戻したい | `git fetch origin && git reset --hard origin/main` |
...
| `pwd` | 現在のディレクトリのパスを表示（`pwd` は `Print Working Directory` の略。`Print` = 表示する、`Working Directory` = 現在作業しているディレクトリ） |
| `cat file` | ファイルの内容を表示 |
...
| `vi'` | `'...'` の中身を選択（引用符は含まない） | **v**isual **i**nner `'` |
| `va'` | `'...'` ごと選択（引用符含む） | **v**isual **a**round `'` |
| `di'` | `'...'` の中身を削除 | **d**elete **i**nner `'` |
| `da'` | `'...'` ごと削除（引用符含む） | **d**elete **a**round `'` |
| `ci'` | `'...'` の中身を削除して挿入モードへ | **c**hange **i**nner `'` |
| `yi'` | `'...'` の中身をコピー | **y**ank **i**nner `'` |
...
| `<leader>tF` | サイドバー表示とフロート表示を切り替え（ノーマルモードのみ） | **t**ree **F**loat（大文字＝表示モード切替） |
| `Alt+f` | サイドバー表示とフロート表示を切り替え（ターミナルモードでも動作、例: Claude Code 入力中） | **f**loat |
...
>
> **補足**: `pptx` / `ppt` / `pdf` は nvim-tree 上に表示され、`Enter` / `Tab` ではエディタに展開せず Windows の既定アプリ（PowerPoint / PDF ビューア等）で開く設定（`explorer.exe` 経由、WSL2用）。
...
スクラッチパッドと違い、ファイルとして残したい一時メモ用。zsh の `memo` コマンド一発でタイムスタンプ付きメモが開き、追加のメモは nvim 内から `<leader>N` で作れる。

| キー / コマンド | 動作 | 由来 |
|---------------|------|------|
| `memo`（zsh） | メモディレクトリ（`~/memo`。`MEMO_DIR` で変更可）に `yyyymmddhhmmss.md` を作成して nvim で開く | **memo** |
| `<leader>N` | カレントディレクトリに `yyyymmddhhmmss.md` を作成して開く | **N**ew note（`<leader>n` の Shift 版） |
...
| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `G` → `P` | 親イシュー（上の階層）に戻る。 | **G**o to **P**arent issue |
...
`~/.ssh/config`（dotfiles 管理: `windows/.ssh/config`）に `Host devbox` を定義しているので、どのツールからも同じ名前で入れる。

| コマンド | 動作 |
|---------|------|
| `ssh devbox` | devbox に SSH（鍵・ユーザー名は config が解決） |
| `ssh devbox -t tmux new -As main` | tmux の main セッションに直接 attach（WezTerm 外から） |
| `scp devbox:~/path/file .` | devbox からファイル取得（アップロードは逆向き） |
| `pwsh -File windows\bin\devbox.ps1 ensure` | VM 起動 + NSG の現在 IP 許可を担保（ネットワーク変更後の復旧はこれ or `<leader> →a`） |
