...
| `Alt+p` | 現在のペインに名前を付ける（空 Enter で解除して自動表示に戻る。`<leader> →t` / `Ctrl+b → Shift+T` でも可） | **p**ane name |
| `Alt+r` / `Alt+Shift+r` | ペインを次/前に回転して入れ替え | **r**otate |
| `<leader> →z` | 現在のペインをズーム（トグル） | **z**oom |
| `<leader> →p` | ペイン番号をオーバーレイ表示して選択 | **p**ane |
...
| `<leader> →o` | **画面上の URL を fzf ポップアップで選んでブラウザで開く**（**o**pen）。tmux 側で折り返しを結合してから抽出するので、**複数行に折り返した URL も完全な形で開ける**。1 件だけなら選択なしで即開く（`tmux-open-url` → SetUserVar `open_url` → WezTerm が既定ブラウザ起動） |
