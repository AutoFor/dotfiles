...
devbox の tmux 上では「タブ」の実体は tmux ウィンドウ（画面下部のステータスラインに表示）になり、以下のキーは自動的に tmux 操作にブリッジされる。ローカルペインでは従来どおり WezTerm のタブを操作する。

| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `Ctrl+t` | 新規タブ（tmux 内は新規ウィンドウ） | ブラウザ由来（**t**ab） |
| `Ctrl+w` | 現在のタブを閉じる（tmux 内はウィンドウを閉じる。確認あり） | ブラウザ由来（**w**indow close） |
| `Ctrl+Tab` | 次のタブへ移動（tmux 内は次のウィンドウ） | ブラウザ由来 |
| `Ctrl+Shift+Tab` | 前のタブへ移動（tmux 内は前のウィンドウ） | 上記の逆 |
| `Ctrl+1`〜`Ctrl+9` | タブ番号で切り替え（tmux 内はウィンドウ番号） | 番号＝タブ位置 |
| `Alt+,` | タブを左に移動（tmux 内はウィンドウ入れ替え） | `,` = `<`（左向き）と同キー |
| `Alt+.` | タブを右に移動（tmux 内はウィンドウ入れ替え） | `.` = `>`（右向き）と同キー |
| `Alt+e` | 現在のタブ名を変更（tmux 内はウィンドウ名変更） | **e**dit name |
| `<leader> →Shift+P` | PowerShell タブを新規で開く | **P**owerShell |
| `<leader> →l` | ランチャーメニューを表示（PowerShell / WSL 等） | **l**aunch |
| `<leader> →a` | Azure devbox の tmux main セッションを新規タブで開く（VM 停止中でも自動起動してから ssh + tmux attach。休止/切断後の復帰はこれ） | **a**zure |
...
| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `<leader> →r` | ペインを左右に分割（devbox の tmux 上では tmux の分割にブリッジ） | **r**ight（左右に分かれる） |
| `<leader> →d` | ペインを上下に分割（devbox の tmux 上では tmux の分割にブリッジ） | **d**own（上下に分かれる） |
| `<leader> →x` | 現在のペインを閉じる（devbox の tmux 上では tmux ペインを閉じる） | × 印（消す） |
| `Alt+h/l/k/j` | 左/右/上/下のペインへ移動（WezTerm → tmux → nvim の順に、ペインが無ければアプリへ透過） | vim の hjkl |
| `Ctrl+Shift+[` | ペインを番号で選択 | vim の `[` ナビゲーション慣習 |
| `<leader> →z` | 現在のペインをズーム（トグル。devbox の tmux 上では tmux の zoom） | **z**oom |
