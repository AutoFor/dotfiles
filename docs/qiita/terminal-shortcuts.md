...
`v` のあとに `i` + 対象キーで「対象の中身」、`a` + 対象キーで「対象まるごと（区切り記号込み）」を選択できる。
文法は「動詞 + i/a + 対象」: `v`（選択）の代わりに `y`（コピー）`d`（削除）`c`（書き換え）も動詞になる（例: `yiw` `diw` `ciw`）。

| キー | 動作 | 由来 |
|-----|------|------|
| `viw` | カーソル位置の単語を選択 | **v**isual **i**nner **w**ord |
| `vi'` / `vi"` | クォートの中身を選択（カーソルと同じ行のみ有効） | **i**nner `'` |
| `vi(` / `vi{` / `vi[` | 括弧の中身を選択（複数行OK・ネストはカーソル位置の内側） | **i**nner `(` |
| `vit` | HTML/XML タグの中身を選択 | **i**nner **t**ag |
| `vip` | 段落（空行区切り）を選択 | **i**nner **p**aragraph |
| `vaw` | 単語を後ろの空白ごと選択 | **a**round **w**ord |
| `va'` / `va(` | クォート／括弧を記号ごと選択 | **a**round `'` |
| `ciw` | 単語を消して Insert モードへ（単語の書き換え） | **c**hange **i**nner **w**ord |
