...
**現在のセッション名はタブバー左端（左上）に青いセグメントで常時表示される**（tmux 側の `wezterm-tabs-sync` がセッション名を通知し、ウィンドウのタブ列と同じ形のリボンとして描画。色だけ青でタブと区別）。
また、**画面下部の tmux ステータスバーの左下に、接続表示（通常は紫の `devbox`、アクティブペインで ssh 実行中は赤の `ssh`）、アクティブペインの cwd のフルパス（青、ホームは `~` 省略）、git ブランチ名（緑）が常時表示される**（リポジトリ外はパスのみ。長いパスは末尾優先で先頭を `…` 省略）。WezTerm 右上は普段は空で、LEADER・音声入力中などの一時的なインジケータだけが出る（音声入力の録音状態は Windows 側にしか無いため右上に残している）。ウィンドウ一覧・右下（セッション名+日時）は WezTerm のタブバーと重複するため、モバイル（グループセッション）のときだけ下部バーに表示される。

| ショートカット / コマンド | 動作 | 由来 |
|------------------------|------|------|
| `tm` | main セッションに attach（無ければ作成）。シェルで実行 | **t**mux **m**ain |
| `tm <名前>` | 別セッションに attach（無ければ作成） | 同上 |
| `<leader> →s` | セッション一覧から選択して切り替え（名前順＝並び順番号の順で出る） | **s**ession |
...
| `Alt+K` / `Alt+J` | 前 / 次のセッションへ（1 発。`Ctrl+q → (` / `)` と同じ。旧 `Ctrl+Alt+K/J`） | k=上=前, j=下=次 の vim 方向 |
| `Alt+↑` / `Alt+↓`（`Ctrl+↑/↓`） | **ペインの上端/下端まで来てからもう 1 回押すと**前 / 次のセッションへ抜ける（移動先では反対側の端のペインに着地）。ペイン移動とセッション移動が同じキーで繋がっているので、「縦に押し続けると全セッションを一周」できる。1 発で飛びたいときは `Alt+K/J` | 縦軸 = セッション |
| `Ctrl+q → (` / `)` | 前 / 次のセッションへ | tmux 標準 |
| `<leader> →T` | **現在のセッション名を変更**（番号は維持されるので入力するのは名前だけ。`3-main` のように先頭に番号を書くと**その位置へ移動**する。`Ctrl+q → $`） | **T**itle（`<leader> →t` のタブ名変更のセッション版＝大文字） |
| `Alt+Shift+,` / `Alt+Shift+.` | **セッションを並びの前 / 後ろへ入れ替える**（`Alt+,` / `Alt+.` のタブ入れ替えのセッション版。端まで来たらそれ以上は動かない） | `<` `>` の向き |
| `Ctrl+q → d` | セッションから detach（セッションは生存継続） | **d**etach |

**セッションの並び順は名前の先頭に付く 2 桁の番号（`01-main` `02-dotfiles` …）で決まる。**
tmux はセッションを名前の昇順でしか並べない（`Alt+K/J` の巡回順もこの順）ため、
番号を振って並びを操作できるようにしている。番号は自動で採番・詰め直しされるので手で付ける必要はなく、`Alt+Shift+,` / `Alt+Shift+.` で隣と入れ替えるか、
`<leader> →T` の入力で `3-main` のように**先頭に位置番号を書いて一発で移す**かの 2 通りで並び替える
（`3-main` は「3 番目に置く」の意味。実際に付く番号は移動後に 01, 02, … と振り直される。
`9-main` のように数が多すぎるときは末尾に丸められ、`2024-plans` のような 3 桁以上は番号ではなく名前として扱う）。
`tm main` や WezTerm 起動時の attach は番号無しの名前でも実際のセッション（`03-main`）を引くので、
これまでどおりの打ち方で入れる。
`<leader> →s` の一覧も同じ名前順にしてある（tmux 3.4 の `choose-tree` は既定だとセッションの
**作成順**で並び、番号と食い違って見えるため、開いた直後に並び替えキー `O` を送って名前順にしている）。
...
> セッション名が変わる操作（`<leader> →T` の改名、`Alt+Shift+,` / `Alt+Shift+.` の並び替え）も位置キーが変わるので、同じく自動で保存し直す。
...
ウィンドウが多くてタブバー幅に収まらないときは、**アクティブなタブを中心に、各タブの実際の名前幅で詰められるだけ並べ**、溢れた分を左右の `+N`（隠れているタブ数）で示す。短い名前のタブは場所を取らないので、同じ幅でもなるべく多く表示される。全部見えないときはウィンドウを広げるか、`Ctrl+数字` で目的のタブへ直接飛ぶ。

| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `Ctrl+t` | 新規タブ（ローカルペイン＝PowerShell 等では同じドメインの WezTerm タブを新規作成） | ブラウザ由来（**t**ab） |
| `Ctrl+w` | 現在のタブを閉じる（確認なしで即クローズ。ローカルペインでは WezTerm タブを閉じる） | ブラウザ由来（**w**indow close） |
| `Ctrl+Shift+w` | **閉じたタブを開き直す**。タブ名・ペインのレイアウト・各ペインの cwd とペイン名が戻り、claude が動いていたペインは同じ会話で再開する。連打すると閉じた順に遡れる（直近 20 件まで） | `Ctrl+w` の取り消し（Shift＝逆操作） |
...
| `Alt+h/l` | 左/右のペインへ移動（端まで来たら nvim 等のアプリへ透過）。上下は `Alt+↑/↓` か `Ctrl+↑/↓` | vim の hl |
| `Alt+←/→`（`Ctrl+←/→`） | 左/右のペインへ移動。**端で止まらずタブ（tmux ウィンドウ）を跨ぐ**（一番左で `Alt+←` → 前のタブの一番右のペイン、一番右で `Alt+→` → 次のタブの一番左のペイン）。タブの端でも巻き戻るので押し続けると全タブの全ペインを一周できる。右列が上下分割されたタブに左から入ると下側のペインに着く（tmux が指定できる角が `{bottom-right}` のため） | 矢印キー = 方向 |
| `Alt+↑/↓`（`Ctrl+↑/↓`） | 上/下のペインへ移動。**端で止まらずセッションを跨ぐ**（一番上で `Alt+↑` → 前のセッションの一番下のペイン、一番下で `Alt+↓` → 次のセッションの一番上のペイン）。セッションの並びは名前順（`Alt+K/J` の巡回と同じ順）で、端でも巻き戻る。**横軸＝タブ、縦軸＝セッション**と覚える | 矢印キー = 方向 |
...
| `Ctrl+b → q` | ペイン番号をオーバーレイ表示して選択（`<leader> →q` は音声入力トグルに変更） | tmux の `prefix+q` と同じ |
...
| ペイン間移動（vim 風） | `Alt+h/l`（`Alt+矢印` でも可。`Alt+k/j` はセッション切替に割当済み） | 同上 |

#### タッチ操作（`mouse on` で tmux が反応する）

> **現在は無効 (2026-08-22)**: Claude Code のプロンプト枠クリックで罫線 `─` が
> クリップボードに入る誤爆が多いため `.tmux.conf` を `mouse off` にした。
> タッチ操作を使いたくなったら `mouse on` に戻す（トレードオフはこの誤爆の再発）。
...
| タブ名変更 | `,` |
| タブを閉じる | `&`（確認なしで即クローズ） |
| 閉じたタブを開き直す | `W`（Shift+w。連打で閉じた順に遡る） |
| ペイン分割（左右 / 上下） | `\|` / `-` |
| ペイン間移動 | 矢印キー |
| ペイン番号を表示して選択 | `q` → 番号 |
...
（`mouse off` のため、以前のようにホイールスクロールで自動的に copy-mode には入らない。
履歴をさかのぼるときはこの copy-mode を使う）。キーは vi 準拠:
...
`mouse off` (2026-08-22〜) のため、マウス操作は tmux を素通りして **WezTerm ネイティブ選択**になる:

| 操作 | 動作 |
|------|------|
| ドラッグ / ダブルクリック / トリプルクリック | WezTerm ネイティブの範囲/単語/行選択。コピーは `Ctrl+Shift+C`（tmux 時代の「離した瞬間に自動コピー」は無効）。選択はペイン border やステータスラインをまたぐので、複数ペイン表示中は罫線が混入することに注意（`prefix z` でズームしてから選ぶと安全） |
...
| `Ctrl`+クリック | カーソル下の **URL を Windows 側の既定ブラウザで開く**。**折り返した URL は先頭行しか検出できない**ので、その場合は `<leader> →o` を使う |

正確な範囲（1ペイン内・折り返し URL 対応・自動でクリップボード行き）が欲しいときは
マウスではなく copy-mode（`prefix [` → `v` → `y`）か `<leader> →u` を使うのが確実。
...
`Ctrl+Space` または `<leader> →q`（Ctrl を押しっぱなしにした `Ctrl+q → q` の連打が確実。IME オン時は素の `q` が IME に吸われて効かないことがある）で録音を開始すると、**話の切れ目（約 0.7 秒の無音）ごとにフレーズ単位で文字起こしされ、フォーカス中のペイン（tmux 内の Claude Code プロンプト等）へ逐次入力される**。終了はもう一度同じキーを押すか、**Enter**（送信と同時に自動停止）。

| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `Ctrl+Space` / `<leader> →q` (1 回目) | 録音開始。右ステータスにマイクアイコンが出て、色が実際の音声に連動する（**灰色=マイク起動待ち、青=録音中だが無音、赤=声を拾っている**。青になってから話すと頭が切れない。話して赤くならないならマイク・しきい値を疑う）。以降、話すたびにフレーズ単位で文字が流れ込む | Space = 話す間 |
| `Ctrl+Space` / `<leader> →q` (2 回目) | 終了（言いかけのフレーズも変換してから止まる） | 同上 |
| `Enter`（録音中） | 通常どおり Enter がペインへ送られ、**同時に録音も終了**する（「話し終わってそのまま送信」で停止し忘れを防ぐ）。停止時点で変換中のフレーズは、変換が済み次第入力される（送信後になることもある） | 話し終わり = 送信 |

MS-IME のキー設定で Ctrl+Space を「IME オン/オフ」に割り当てていると WezTerm に届かず IME 切替になってしまうので、その場合は IME 側の割り当てを外すか `<leader> →q` を使う。

仕組み: `windows/bin/voice-input.ps1` が winmm (`waveIn`) でマイクを録りっぱなしにし、音量 (RMS) ベースの無音検出でフレーズを切り出して Groq (`whisper-large-v3-turbo`) に POST → `wezterm cli send-text` でペインに流し込む（追加ソフト不要）。Groq の Whisper API はストリーミング非対応のため、フレーズ単位の逐次変換で体感を近づけている（真のリアルタイムが欲しいときは Windows 標準の `Win+H` を使う。精度より速度の Win+H、速度より精度の `Ctrl+Space` という使い分け）。直前までの文字起こしを文脈ヒント (`prompt`) として渡すので、フレーズをまたぐ用語も繋がりやすい。
...
- **幻聴フィルタ**: 雑音・ほぼ無音のチャンクで Whisper が出す幻聴（「ご視聴ありがとうございました」等の定型句、直前フレーズの繰り返し、同語反復ループ）は `no_speech_prob` / `compression_ratio` / 定型パターンで自動破棄される（破棄理由はログに残る）。録音プロセスは PID ファイルで単一インスタンス化され、生き残りのゾンビ録音は次回起動時に kill される
- ペインへの入力に失敗したときは、そこまでの全文をクリップボードに退避してトーストで知らせる（`Ctrl+Shift+V` で貼り付け）
- 言語は既定で日本語ヒント付き（`voice-input.ps1` の `-Language` を空にすると自動判定）
- **無音しきい値は自動決定**（`-SilenceThreshold` の既定 `0` = 自動）。マイクの入力ボリュームは環境ごとにまちまちで、固定値だと音量の小さいマイクで発話が「声あり」と判定されず丸ごと取りこぼす。録音開始直後 600ms の環境音からノイズフロアを実測し、その 2 倍を採用する（120〜300 にクランプ。計測中は下限の 120 で判定するので押した直後に話しても頭が切れない）。採用値は `%TEMP%\wezterm-voice.log` に `ノイズフロア=… → 無音しきい値=…` として残る。固定したいときは `-SilenceThreshold 300` のように正の値を渡す
- **送信前に音量を正規化**する。小さいまま投げると Whisper 側の認識精度も落ちるので、Groq へ送る直前にピークがフルスケールの約 67% になるよう増幅する（上限 8 倍、既に十分な音量のチャンクは歪ませないよう無加工）。掛かった倍率はログに残る
...
- 一時的な文字起こし失敗（レート制限・ネットワーク断）はそのフレーズだけ捨てて**録音を継続**する（トーストで知らせる。連続 3 回失敗で終了）。録音プロセスが落ちた場合も約 10 秒でアイコンが消えて状態が自動リセットされ、次の `Ctrl+Space` が普通に効く
...
#### テキストオブジェクト（意味のまとまりで一発選択）

文法は「**動詞 + i/a + 対象**」の 3 パーツ。カーソルが対象のどこにあっても効く。
...
| `y` | コピーする | **y**ank |
| `d` | 削除する | **d**elete |
| `c` | 消して Insert モードへ（書き換え） | **c**hange |
...
| `w` | 単語（**w**ord） | |
| `'` / `"` / `` ` `` | クォート | カーソルと同じ行のみ有効 |
| `(` / `{` / `[` | 括弧 | 複数行OK。ネストはカーソル位置の内側 |
| `t` | HTML/XML タグ（**t**ag） | |
| `p` | 段落＝空行区切り（**p**aragraph） | |
| `s` | 文＝`.` 区切り（**s**entence） | |
...
| キー | 動作 | 読み下し |
|-----|------|---------|
| `viw` | 単語を選択 | **v**isual **i**nner **w**ord |
| `yiw` | 単語をその場でコピー（選択操作なし） | **y**ank **i**nner **w**ord |
| `diw` | 単語を削除 | **d**elete **i**nner **w**ord |
| `ciw` | 単語を書き換え（変数名の修正に最適） | **c**hange **i**nner **w**ord |
| `vi'` / `vi"` | クォートの中身を選択 | **v**isual **i**nner `'` |
| `yi'` | クォートの中身をコピー | **y**ank **i**nner `'` |
| `ci'` | クォートの中身を書き換え（文字列の差し替え） | **c**hange **i**nner `'` |
| `vi(` / `vi{` | 括弧の中身を選択 | **v**isual **i**nner `(` |
| `di(` | 括弧の中身を削除（引数の削除） | **d**elete **i**nner `(` |
| `ci(` | 括弧の中身を書き換え（引数の差し替え） | **c**hange **i**nner `(` |
| `vit` / `cit` | タグの中身を選択／書き換え | **i**nner **t**ag |
| `vip` / `yip` | 段落を選択／コピー | **i**nner **p**aragraph |
| `vaw` | 単語を後ろの空白ごと選択 | **a**round **w**ord |
| `va'` / `da'` | クォートを記号ごと選択／削除 | **a**round `'` |
| `va(` / `da(` | 括弧を記号ごと選択／削除 | **a**round `(` |
