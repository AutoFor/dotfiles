...
- **小さいマイクでも拾う**: 無音判定のしきい値は固定値ではなく、録音開始直後 600ms の環境音から実測したノイズフロアの 3 倍（120〜800 でクランプ）を自動採用する。マイクの入力ボリュームが小さい環境でも「声あり」判定が立つ。計測中は下限値で判定するので頭は切れない。決定したしきい値は `%TEMP%\wezterm-voice.log` に残る
