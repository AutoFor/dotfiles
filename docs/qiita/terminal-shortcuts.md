...
| SSH リモート（Azure devbox 等） | OSC 1337 SetUserVar（`claude_notify`）→ 手元の WezTerm の `user-var-changed` ハンドラが Windows トーストを表示 |

どちらもタイトルに `[ディレクトリ名]`（SSH の場合は `[ホスト名:ディレクトリ名]`）が付くので、複数タブで Claude Code を並行実行していてもどのセッションの通知か区別できる。

SSH リモート側の送信は 2 つの落とし穴に対応している:

- Claude Code の hook プロセスには制御端末が無く `/dev/tty` を開けないため、`$WEZTERM_PANE` から `wezterm cli list` で自ペインの pty を特定してそこに書き込む
- OSC 777 のトーストは mux ドメイン（`wezterm-mux-server` 経由の接続）を越えて手元に届かないため使わず、user var を受けた WezTerm が Lua 側で `window:toast_notification()` を呼ぶ（user var は mux を越えて伝搬する）

受信側（`.wezterm.lua` の `user-var-changed`）では以下が動く:

- Windows トーストの表示（通知元ペインを見ているときは表示しない）
- **トーストをクリックすると通知元ペインへジャンプ**（トーストに `wezterm-pane://<pane_id>` の URL を添え、`install-windows.ps1` が登録するプロトコルハンドラ `windows/wezterm-goto-pane.ps1` が `wezterm cli activate-pane` でペインを切り替えてウィンドウを前面化する。初回は `install-windows.ps1` の再実行が必要）
- 通知元ペインのタブタイトルに `🔔 ディレクトリ名` のマークが付く（そのペインにフォーカスすると元のタイトルに復元）
- `<leader> →j` で「最後に通知が来たペイン」へジャンプできる（別ウィンドウ・別タブでも可。トーストが消えた後でも使える）
...
### tmux セッション永続化（Azure devbox）

VM の再起動・deallocate で tmux セッション（ウィンドウ構成・ペイン分割・作業ディレクトリ）はメモリごと消えるため、tmux-resurrect + tmux-continuum で定期保存・自動復元する。

| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `Ctrl+b → Ctrl+s` | セッション構成を手動保存（シャットダウン前に打つと確実） | **s**ave |
| `Ctrl+b → Ctrl+r` | 保存した構成を手動復元 | **r**estore |

- 自動保存: cron が 15 分ごと + 21:59（22:00 の Azure 自動シャットダウン直前）に保存（continuum の自動保存は `status off` 構成では動かないため cron で代替。bootstrap.sh が登録）
- 自動復元: VM 起動後に SSH で入るだけで前回の構成が復元される（SSH ログイン時に `.zshrc` が自動で tmux(main) へ attach + continuum の起動時復元。手動の `tmux new -As main` は不要）
- 自動 attach の除外: WezTerm の mux ドメイン / `wezterm ssh` のペイン（`WEZTERM_PANE` で判定）では発動しない。素のシェルが欲しいときは `ssh -t <host> NOTMUX=1 zsh -l`
- claude が動いていたペインは `claude --continue` で同じ会話の続きから自動再開する
- 保存先: `~/.local/share/tmux/resurrect/`（設定は dotfiles の `linux/.tmux.conf`）
- アイドル自動停止: 全端末に 1 時間入出力がなく低負荷なら、tmux 構成を保存して VM が自動 deallocate される（`idle-shutdown`、cron 10 分ごと）。Claude Code が出力を流している間はペインの pts が更新されるため「処理中」は停止されない。判定ログは `~/.cache/idle-shutdown.log`
