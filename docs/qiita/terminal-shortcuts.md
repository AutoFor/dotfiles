...
`Shift+↓` か `Ctrl+Space`（ほかに `<leader> →q` でも可。Ctrl を押しっぱなしにした `Ctrl+q → q` の連打が確実。IME オン時は素の `q` が IME に吸われて効かないことがある）で録音を開始すると、**話の切れ目（約 0.7 秒の無音）ごとにフレーズ単位で文字起こしされ、フォーカス中のペイン（tmux 内の Claude Code プロンプト等）へ逐次入力される**。もう一度同じキーで終了。

| ショートカット | 動作 | 由来 |
|--------------|------|------|
| `Shift+↓` / `Ctrl+Space` / `<leader> →q` (1 回目) | 録音開始。右ステータスにマイクアイコンが出て、色が実際の音声に連動する（**灰色=マイク起動待ち、青=録音中だが無音、赤=声を拾っている**。青になってから話すと頭が切れない。話して赤くならないならマイク・しきい値を疑う）。以降、話すたびにフレーズ単位で文字が流れ込む | Space = 話す間 |
| `Shift+↓` / `Ctrl+Space` / `<leader> →q` (2 回目) | 終了（言いかけのフレーズも変換してから止まる） | 同上 |
