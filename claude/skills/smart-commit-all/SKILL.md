---
name: smart-commit-all
description: リポジトリ全体の変更を分析し、テーマごとにコミットを分割して作成する。
...
リポジトリ内のすべての変更を対象に、意味のある単位でコミットを分割して作成する。
複数セッションの変更も含めて一括処理したい場合に使う。
...
- **他のスキルから呼び出された場合**: コミット完了後、呼び出し元スキルの次のステップに処理を戻すこと。
