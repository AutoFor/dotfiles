...
> **CRITICAL: このステップは省略不可。** Worktree 使用時にスキップすると、cleanup で CWD が無効化され以降の**全コマンドが失敗する**。CWD がサブディレクトリの場合も同様に問題が発生するため、常に実行すること。
...
`pwd` の出力がメインリポジトリのパスと一致することを確認してから次のステップに進む。
...
**例:**
```bash
bash ~/.codex/skills/gh-pr-approve/approve-pr.sh <owner> my-repo 44
```
...
```bash
bash ~/.codex/skills/gh-pr-approve/cleanup-after-merge.sh <メインリポジトリの絶対パス> <Worktreeの絶対パス> master <ブランチ名>
```

**通常ブランチの場合:**
```bash
bash ~/.codex/skills/gh-pr-approve/cleanup-after-merge.sh <リポジトリの絶対パス> none master <ブランチ名>
```
...
bash ~/.codex/skills/gh-pr-approve/cleanup-after-merge.sh /home/user/projects/codex-config /home/user/projects/codex-config-feature master
```
...
A: 以下を確認してください：
1. `~/.codex/github-app-config.env` の Client ID と Installation ID が正しいか
2. `~/.codex/github-app-key.pem` が存在し、正しい秘密鍵か
