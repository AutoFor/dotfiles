...
flow_a() {
  ...
  # [codex exec #1] Issue タイトル + 英語スラッグ（1回のみ）
  echo "Issue タイトルを生成中..."
  local claude_out issue_title slug
  claude_out=$(printf '%s\n%s\n%s' "$diff_all" "$log" "$status" | codex exec \
  ...
  # Issue 作成
  echo "Issue を作成中..."
  local issue_url issue_num issue_body_file
  issue_body_file=$(mktemp)
  build_issue_body "$status" "$diff_all" "$log" "$issue_body_file"
...
# ─── PR を Ready for Review に変更 [codex exec #3] ────────────────────
ready_for_review() {
  local issue_num="$1"
  local branch
...
approve_and_merge() {
  ...
  local approve_script="$HOME/.codex/skills/gh-pr-approve/approve-pr.sh"
...
cleanup() {
  ...
  local cleanup_script="$HOME/.codex/skills/gh-pr-approve/cleanup-after-merge.sh"
